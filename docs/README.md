...
| [0008](adr/0008-hand-written-css-themes.md) | **proposed** | One stylesheet `shop/static/shop/site.css`; colours as `:root` custom properties; light + dark via `prefers-color-scheme` and `data-theme`; `theme.js` toggle | Do not add a second stylesheet, an external font or a CDN stylesheet. Mobile-first. Not yet accepted, so do not treat it as settled | `tests/e2e/test_theme.py` (overflow, 44×44 targets, 4.5:1 contrast); nothing checks the one-stylesheet rule |
