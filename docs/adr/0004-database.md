...
Implemented on 2026-09-14. Local development runs a PostgreSQL 17 cluster started with `python scripts/dev.py db:start`; `.env.example` carries the connection string and notes that the local cluster's `trust` authentication is a development-only convenience that must not reach a deployed environment.
