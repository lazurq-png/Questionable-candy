...
def confirm(client, **changes):
    ...
    `shown` is the fingerprint the page last shown would post, unless given.
    """
    shown = snapshot_fingerprint(client.session[SESSION_KEY]["showing"])
    data = {"csrfmiddlewaretoken": client.token, "shown": shown, **ALL_THREE, **changes}
...
def test_all_three_confirmations_confirm_the_order(acknowledged):
    """Recorded with when, and the page then says it is confirmed."""
    response = confirm(acknowledged)

    assert response.status_code == 302
    assert response["Location"] == CONFIRM
    confirmed = acknowledged.session[SESSION_KEY]["confirmed"]
    assert confirmed["snapshot"] == acknowledged.session[SESSION_KEY]["showing"]
    assert confirmed["at"]
    page = acknowledged.get(CONFIRM).content.decode()
    assert 'data-testid="confirm-done"' in page
    assert 'data-testid="confirm-form"' not in page
...
def test_any_one_confirmation_missing_or_wrong_is_refused(acknowledged, change):
    ...
    assert "confirmed" not in acknowledged.session[SESSION_KEY]
...
def test_a_price_change_while_confirming_is_refused_and_the_new_order_shown(acknowledged):
    ...
    assert confirm(acknowledged, typed_total="12.00").status_code == 302  # the new order, confirmed


def test_changing_the_cart_after_confirming_starts_again_at_the_warning(acknowledged):
    """UC-08: the exact cart contents are what was confirmed."""
